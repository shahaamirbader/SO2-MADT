...
from models.DT_so2madt_model import DT, DTConfig
...
parser.add_argument('--offline_data_dir', type=str, default='/home/xubuntu/Desktop/data/3rd_Year/DT_RL/MADT/sc2/data/')
...
# STG and APM specific arguments
parser.add_argument('--max_safety_limit', type=float, default=10.0)
parser.add_argument('--safety_penalty', type=float, default=1.0)
...
# Include STG token dimension
stg_dim = 4  # One dimension for each safety criterion

block_size = args.context_length * 5  # Adjusted for STG tokens

print("global_obs_dim: ", global_obs_dim)
print("local_obs_dim: ", local_obs_dim)
print("action_dim: ", action_dim)
print("stg_dim: ", stg_dim)

mconf_actor = DTConfig(local_obs_dim, action_dim, block_size,
                        n_layer=2, n_head=2, n_embd=32, model_type=args.model_type,
                        max_timestep=args.max_timestep, stg_dim=stg_dim)
model = DT(mconf_actor, model_type='actor')

mconf_critic = DTConfig(global_obs_dim, action_dim, block_size,
                         n_layer=2, n_head=2, n_embd=32, model_type=args.model_type,
                         max_timestep=args.max_timestep, stg_dim=stg_dim)
critic_model = DT(mconf_critic, model_type='critic')
...
buffer = ReplayBuffer(block_size, global_obs_dim, local_obs_dim, action_dim, stg_dim, args.max_safety_limit)
rollout_worker = RolloutWorker(model, critic_model, buffer, global_obs_dim, local_obs_dim, action_dim, stg_dim, args.max_safety_limit)
...
offline_tconf = TrainerConfig(max_epochs=1, batch_size=args.offline_mini_batch_size, learning_rate=args.offline_lr,
                              num_workers=0, mode="offline", lambda_reg=args.lambda_reg,
...
online_tconf = TrainerConfig(max_epochs=args.online_ppo_epochs, batch_size=0,
                             ...
                             lambda_reg=args.lambda_reg, safety_penalty=args.safety_penalty)
