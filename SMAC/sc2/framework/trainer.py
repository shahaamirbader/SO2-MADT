...
class Trainer:
    ...
    def train(self, dataset, train_critic=True):
        ...
        def run_epoch():
            ...
            for it, (s, o, a, r, ava, v, rtg, ret, adv, t, pre_a, next_s, next_rtg, done, stg, safety_violation) in pbar:
                s = s.to(self.device)
                o = o.to(self.device)
                a = a.to(self.device)
                r = r.to(self.device)
                ...
                stg = stg.to(self.device)
                safety_violation = safety_violation.to(self.device)

                # Compute attention weights based on advantages
                adv = adv.reshape(-1, adv.size(-1))
                attention_weights = F.softmax(adv, dim=0)
                weighted_adv = adv * attention_weights

                # Update actor
                with torch.set_grad_enabled(True):
                    logits = model(o, pre_a, rtg, t, stg)
                    if self.config.mode == "offline":
                        cross_entropy_loss = F.cross_entropy(logits.reshape(-1, logits.size(-1)), a.reshape(-1))
                        safety_loss = self.config.lambda_reg * torch.mean(torch.sum(safety_violation, dim=1))
                        loss = cross_entropy_loss + safety_loss
                        entropy_info = 0.
                        ratio_info = 0.
                        confidence_info = 0.
                    elif self.config.mode == "online":
                        ...
                        old_logits = target_model(o, pre_a, rtg, t, stg).detach()
                        ...
                        actor_loss_ori = imp_weights * weighted_adv
                        actor_loss_clip = torch.clamp(imp_weights, 1.0 - 0.2, 1.0 + 0.2) * weighted_adv
                        actor_loss = -torch.min(actor_loss_ori, actor_loss_clip)

                        act_entropy = distri.entropy().unsqueeze(-1)
                        safety_loss = self.config.lambda_reg * torch.mean(torch.sum(safety_violation, dim=1))
                        loss = actor_loss - 0.01 * act_entropy + safety_loss
                ...
                if train_critic:
                    with torch.set_grad_enabled(True):
                        v_value = critic_model(s, pre_a, rtg, t, stg)
