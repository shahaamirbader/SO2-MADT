...
class StateActionReturnDataset(Dataset):
    ...
    def __init__(self, global_state, local_obs, block_size, actions, done_idxs, rewards, avas, v_values, rtgs, rets,
                 advs, timesteps, stgs, safety_violations):
    ...
    def __getitem__(self, idx):
        context_length = self.block_size // 5  # Adjusted for STG tokens
        ...
        stgs = torch.tensor(self.stgs[idx:done_idx], dtype=torch.float32)
        safety_violations = torch.tensor(self.safety_violations[idx:done_idx], dtype=torch.float32)

        dones = torch.zeros_like(rewards)
        if done_idx in self.done_idxs:
            dones[-1][0] = 1

        return states, obss, actions, rewards, avas, v_values, rtgs, rets, advs, timesteps, pre_actions, next_states, next_rtgs, dones, stgs, safety_violations

class ReplayBuffer:

    def __init__(self, block_size, global_obs_dim, local_obs_dim, action_dim, stg_dim, max_safety_limit):
        self.block_size = block_size
    ...
    def insert(self, global_obs, local_obs, action, reward, done, available_actions, v_value, safety_violations):
            ...
            for i in range(offline_episode_num[j]):
                ...
                for agent_trajectory in episode:
                    for step in agent_trajectory:
                        ...
                        # Initialize safety violations to zero for offline data
                        step.append([0.0 for _ in range(self.stg_dim)])
    ...
    def sample(self):
        ...
        for episode_idx in range(self.size):
            ...
            for agent_trajectory in episode:
                time_step = 0
                accumulated_safety = np.zeros(self.stg_dim)
                for step in agent_trajectory:
                    g, o, a, r, d, ava, v, safety_violation, rtg, ret, adv, stg = step
                    accumulated_safety += np.array(safety_violation)
                    stg_token = self.max_safety_limit - accumulated_safety
        ...
        dataset = StateActionReturnDataset(global_states, local_obss, self.block_size, actions, done_idxs, rewards,
                                           avas, v_values, rtgs, rets, advs, time_steps, stgs, safety_violations)
        return dataset
    ...
    def get_episode(self, index):
        ...
        for agent_trajectory in episode:
            ...
            for i in reversed(range(len(agent_trajectory))):
                if len(agent_trajectory[i]) == 7:
                    agent_trajectory[i].append([0. for _ in range(self.stg_dim)])  # Initialize safety violations
                reward = agent_trajectory[i][3][0]
                safety_violation = agent_trajectory[i][7]
                rtg += reward
                ...
                accumulated_safety += np.array(safety_violation)
                stg_token = self.max_safety_limit - accumulated_safety
                agent_trajectory[i].append(stg_token.tolist())
